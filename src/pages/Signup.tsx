...
const Signup = () => {
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<{ fullname: string; email: string; password: string }>();
  ...
  const onSubmit = async ({ fullname, email, password }: { fullname: string; email: string; password: string }) => {
    try {
      ...
      <div className="flex-1 flex items-center justify-center py-20 px-4">
        <Card className="w-full max-w-md shadow-2xl border-0 border-t-4" style={{ borderColor: '#1D6B73' }}>
          <CardHeader
            ...
            style={{ background: 'linear-gradient(90deg, #1D6B73 0%, #155159 100%)' }}
          >
